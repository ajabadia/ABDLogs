...
export function TelemetryDashboard({ tenantId }: { tenantId: string }) {
  ...
  useEffect(() => {
    async function fetchTelemetry() {
      ...
      try {
        ...
        console.error(err);
        toast.error('Error sincronizando telemetría visual.');
      } finally {
        setLoading(false);
      }
    }
  ...
  if (loading && data.length === 0) {
    return (
      <div className="h-[500px] border border-border bg-card/30 rounded-xl flex items-center justify-center">
  ...
  const aggregatedByDate = data.reduce((acc, curr) => {
    ...
    if (curr.action.includes('FAIL') || curr.action.includes('ERROR') || curr.action.includes('DENIED')) {
      acc[curr.date].errors += curr.count;
    }
    return acc;
  ...
  // Process data for App Distribution Chart
            ...
            {[7, 15, 30, 90].map((d) => (
               <button
                 ...
                 className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${
                   days === d ? 'bg-primary text-primary-foreground shadow-sm' : 'hover:bg-secondary text-muted-foreground'
